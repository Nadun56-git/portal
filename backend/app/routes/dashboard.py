...
async def get_dashboard_summary():
    total_clients           = await clients_collection.count()
    total_suppliers         = await suppliers_collection.count()
    total_products          = await products_collection.count()
    total_orders            = await orders_collection.count()
    pending_orders          = await orders_collection.count({"status": "pending"})
    total_purchase_orders   = await purchase_orders_collection.count()
    pending_pos             = await purchase_orders_collection.count({"status": "pending"})

    completed_orders = await orders_collection.find_with_filter({"status": "completed"})
    revenue = sum(o.get("total_amount", 0) for o in completed_orders)

    received_pos = await purchase_orders_collection.find_with_filter({"status": "received"})
...
async def get_monthly_report(year: int = None, month: int = None):
    ...
    all_orders   = await orders_collection.find_all()
    all_pos      = await purchase_orders_collection.find_all()
    all_clients  = await clients_collection.find_all()
    all_suppliers= await suppliers_collection.find_all()
...
async def get_yearly_report(year: int = None):
    ...
    all_clients   = await clients_collection.find_all()
    all_suppliers = await suppliers_collection.find_all()
    all_products  = await products_collection.find_all()
